...
function SingIn() {
  const {loginWithEmailPass}=useContext(ProviderContext)
  ...
  const hendalLogin=(e)=>{
    ...
    .then(res=>{
      const loginTime= res.user?.metadata?.lastSignInTime
      const userInfo={email, loginTime}
  ...
  return (
    <div className="w-full max-w-md p-6 m-auto mx-auto my-10 bg-white rounded-lg shadow-md dark:bg-gray-800">
      <div className="flex justify-center mx-auto">
      <h1 className='text-3xl font-semibold text-center'>Login Account</h1>

      </div>

      <form onSubmit={hendalLogin} className="mt-6">
        <div>
      ...
      <p className="mt-8 text-base font-light text-center text-gray-400">Already have an account
      <Link to='/singup' className="font-medium text-gray-700 dark:text-gray-200 hover:underline"> Create One </Link></p>
    </div>
  );
}
